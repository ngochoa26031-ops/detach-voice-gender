...
- **Voice DNA** — [`speechbrain/spkrec-ecapa-voxceleb`](https://huggingface.co/speechbrain/spkrec-ecapa-voxceleb)
  tạo dấu vân tay giọng cho từng speaker rồi so với các mẫu đã verify trong
  `voice_dna/`. DNA là nguồn duy nhất quyết định Nam/Nữ.
- Speaker không đạt ngưỡng khớp DNA hoặc còn mơ hồ giữa DNA Nam/Nữ được ghi
  vào `Không rõ`; tool không fallback về kết quả gender model cũ.
- Mỗi block SRT được gán vào turn diarization overlap nhiều nhất, rồi thừa
  hưởng kết quả DNA của speaker đó.
...
- `<tên_srt>_speaker.txt` — danh sách range block theo từng speaker, ví dụ:
  `SPEAKER_12 | Nam | dna VOICE_004_NAM_CHINH_SEED score 0.734 margin 0.152 | blocks 1347-1348, 1354`
  `score` là cosine similarity với DNA gần nhất, không phải xác suất đúng.
