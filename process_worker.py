...
def main():
    ...
    def report(msg):
        ...
        print(f"[{_ts()}] [{episode_name}] {msg}", flush=True)
    ...
    def heartbeat():
        while not done.wait(heartbeat_sec):
            ...
            print(
                f"[{_ts()}] [{episode_name}] van dang chay {mins}m{secs:02d}s - "
                f"{last_status['text']}",
                flush=True,
            )
    ...
    try:
        ...
        print(f"[{_ts()}] [{episode_name}] XONG: {txt_path} | {srt_out_path}", flush=True)
    except Exception as exc:
        done.set()
        print(f"[{_ts()}] [{episode_name}] LOI: {exc}", flush=True)
        traceback.print_exc()
        sys.exit(1)
